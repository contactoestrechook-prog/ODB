...
export async function GET(req: Request) {
  ...
  // params extra (p.ej. sesionId de movimientos) se reenvían al backend
  const url = new URL(req.url);
  url.searchParams.delete('recurso');
  const qs = url.searchParams.size ? `?${url.searchParams.toString()}` : '';
  const res = await fetch(`${API}${ruta}${qs}`, { headers: token ? { Authorization: `Bearer ${token}` } : {}, cache: 'no-store' });
...
export async function POST(req: Request) {
  ...
  const ruta = accion === 'abrir' ? '/caja/abrir' : accion === 'cerrar' ? '/caja/cerrar' : accion === 'arca' ? '/arca/emitir' : accion === 'movimiento' ? '/caja/movimiento' : accion === 'autorizar' ? '/caja/autorizar' : null;
